...
from  LSE_CALC_GR import LSE_Calculation
from SSR_calc import SSR_Calc
from Prediction import prediction

def MLR_gr(training_data,test_data,K_FOLD_SIZE,check):
	SSR_list = []
	SSE_list = []
	SST_list = []
	# ro=0
	...
	for i in range(0,K_FOLD_SIZE):
		...
		# print(data)
		LSE, Y_actual_, data,norm = LSE_Calculation(training_data[i],check)
		LSE_list.append(LSE)
	...
	print("For Training Data")
	k = ANOVA_CALC(SSE_list1, SST_list1, SSR_list1, R_sqaured_list1, MSE_list1, MSR_list1, F_list1,
				   Adjusted_R_squared_list1, K_FOLD_SIZE)

	print("For Test Data")
	k_test = ANOVA_CALC(SSE_list, SST_list, SSR_list, R_sqaured_list, MSE_list, MSR_list, F_list,
						Adjusted_R_squared_list, K_FOLD_SIZE)
	return LSE_list, column, k, k_test
